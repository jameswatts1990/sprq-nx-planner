...
- `services/cell_service.py::current_location()` (`cell_service.py:58-65`) derives a cell's pinned instrument from its most recent active `CellUse` → `Cycle` → `RunBatch` → `Instrument.serial_number`. A `Cell` has no direct FK to `Instrument`; location is always derived, never stored, so there's no stale pin state to fall out of sync.
- `services/placement_service.py::place_sample()` (`placement_service.py:164-170`) — manual/drag-drop placement — raises a 409 if a cell's current instrument doesn't match the target instrument.
- `engine/slot_scheduling.py::fill_slots()` (`slot_scheduling.py:48-52`) — the auto-fill packing loop — skips any slot whose instrument doesn't match `cell.pinned_instrument_serial`, falling the sample to `unplaced` rather than crossing instruments even when a slot elsewhere is free.
...
Covered by unit tests in `tests/unit/test_slot_scheduling.py`: `test_fill_slots_respects_cross_instrument_pin_when_a_compatible_slot_exists`, `test_fill_slots_strands_pinned_cell_when_only_a_different_instrument_slot_is_offered`, and `test_fill_slots_unpinned_cell_can_use_any_offered_instrument` (confirms fresh cells remain free to land anywhere).

**If this ever needs to change** (e.g. to support an operator physically relocating a multi-use cell to a different instrument), it requires a deliberate design decision, not a one-line relaxation: `slot_scheduling.py:51` and `auto_fill_service.py:101-104` would both need to relax together, `placement_service.py:165` would need an explicit override path, and `current_location()`/`pinned_instrument_serial` would need a defined meaning for "reassignment" — today the model has no way to represent a cell moving mid-lifecycle short of falsifying its `CellUse` history.
