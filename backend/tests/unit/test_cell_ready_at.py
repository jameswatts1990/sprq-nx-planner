...
free yet" check layered on cell_timing's per-cell-use movie-end model. A cell is free for its next
use once its prior movie ends (the on-board reuse wash is the next use's own prep now, not a gap
here). Neither function ever rejects a placement - see docs/pacbio-sprq-nx-scheduling-reference.md's
"Deliberate simplifications" and auto_fill_service.AutoFillResult.reuse_timing_flags."""
...
# +28h = Tuesday 16:00; the cell is free for its next use then (the wash is the reuse's own prep).
USE1_READY_AT = NOON_MON + timedelta(hours=PREP_H + 24)
...
def test_cell_ready_at_is_the_prior_movie_end():
...
def test_reuse_not_ready_hours_flags_a_premature_second_use():
    use1 = _cell_use(1, MONDAY, NOON_MON)
    # Tuesday 08:00 - well before use1's real movie end of Tuesday 16:00.
...
def test_reuse_not_ready_hours_none_when_start_is_safely_after_ready():
    use1 = _cell_use(1, MONDAY, NOON_MON)
    # Wednesday noon - safely after use1's real movie end of Tuesday 16:00.
    use2 = _cell_use(2, WEDNESDAY, datetime(2026, 8, 5, 12, tzinfo=timezone.utc))
    _cell([use1, use2])
    assert reuse_not_ready_hours(use2) is None


def test_reuse_not_ready_hours_none_at_exactly_the_prep_aware_ready_time():
    # A reuse scheduled EXACTLY at the cell's prep-aware free time (Tue 16:00) reports no shortfall.
    # This is precisely what reuse_plate_window now produces (load + prep + movie), so an ordinary
    # reuse never misfires the "not ready" advisory the way the old prep-blind chain did (~4h).
    use1 = _cell_use(1, MONDAY, NOON_MON)
    use2 = _cell_use(2, TUESDAY, USE1_READY_AT)  # Tue 16:00
    _cell([use1, use2])
    assert reuse_not_ready_hours(use2) is None
